...
pub mod pallet {
	...
	const PRICE_REQUEST: &str = "https://api.coincap.io/v2/assets/polkadot";
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight(10000)]
		pub fn submit_price_unsigned_with_signed_payload(origin: OriginFor<T>, payload: PricePayload<T::Public>,
			_signature: T::Signature) -> DispatchResult
		{
			let _ = ensure_none(origin)?;
			// we don't need to verify the signature here because it has been verified in
	...
	impl<T: Config> Pallet<T> {
		...
		fn fetch_price_info() -> Result<(), Error<T>> {
			...
			match Self::fetch_n_parse_price_info() {
				Ok(price_d) => {
					let PriceData { priceUsd } = price_d;
					let parts = str::from_utf8(&priceUsd).unwrap();

					let parts_arr = parts.split(".").collect::<Vec<&str>>();
					let int_part = parts_arr[0];
					let decimal_part = parts_arr[1];

					log::info!("{}, {}", &int_part, &decimal_part);

					let int_part_num = int_part.parse::<u64>().unwrap();
					let decimal_part_num = decimal_part.get(0..6).unwrap().parse::<u32>().unwrap();

					log::info!("{}, {}", &int_part_num, &decimal_part_num);

					let price = (int_part_num, Permill::from_parts(decimal_part_num));
		...
		fn fetch_n_parse_price_info() -> Result<PriceData, Error<T>> {
		...
		fn fetch_price_from_remote() -> Result<Vec<u8>, Error<T>> {
			log::info!("sending request to: {}", PRICE_REQUEST);

			let request = rt_offchain::http::Request::get(PRICE_REQUEST);
